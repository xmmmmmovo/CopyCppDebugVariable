...
- **String-aware leaf handling** — `std::string`, `std::u8string`, `std::wstring`, `std::u16string`, `std::u32string`, `std::string_view`, `std::pmr::*`, ABI-tagged `std::__cxx11::basic_string<…>` / `std::__1::basic_string<…>`, `const char *`, `char[N]`, `wchar_t *`, `std::byte[N]` / `std::byte *` (e.g. PMR back-buffer), etc. are treated as leaves. The leaf `value` is, in priority order: `""` for an empty container (`indexedItems === 0`); the full UTF-8 / UTF-16 text reconstructed from char / byte children when the adapter exposes them; bytes decoded as UTF-8 when the adapter drops a cppvsdbg byte dump (`0x… {NN '?', …, …}`) into `value` (covers `std::byte[N]` / `std::byte *` when no children are exposed); nothing when the adapter only exposes a truncated preview. No internal `[size]` / `[capacity]` / `[allocator]` leakage either way. For `std::byte[N]` / `std::byte *` the reader pulls the full buffer (via `variablesReference` paging, `readMemory`, format-spec `evaluate`, or per-index `evaluate`) and, by default (`mergeByteBufferIntoValue`), merges it into a single `value` string when the bytes decode as UTF-8 text — binary buffers materialize as `[0]…[N-1]` byte children instead. Turn the setting off to always materialize children.
...
| `mergeByteBufferIntoValue` | `true` | —          | Merge `std::byte[N]` / `std::byte *` byte-buffer content into a single `value` string (string-like leaf) when the bytes decode as UTF-8 text; binary buffers fall back to materialized byte children. Turn off to always materialize all byte children. |
